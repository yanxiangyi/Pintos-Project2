...
void countArgc(char *token, const char *copy, char **save_ptr, int *argc);

void pushStack(void **esp, int *pt);
...
setup_stack(void **esp, char *file_name) {
    ...
    for (i = argc - 1; i >= 0; i--) {
        pushStack(esp, &argv[i]);
    }

    int esp_copy = *esp;
    pushStack(esp, &esp_copy);
    pushStack(esp, &argc);
    pushStack(esp, &zero);
...
void pushStack(void **esp, int *ptr) {
    *esp -= sizeof(int);
